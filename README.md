...
- 用户说“做一个较长实现/重构/审计”，Codex 调用 `companion_start`，然后用返回的 `eventCursor` 调用 `companion_wait` 等进展。
...
| `companion_wait` | 等待长任务的新事件、完成或超时，用于 `start → wait` 的流式监控 |
...
| `bypass` | 显式跳过 Claude Code 权限确认；SDK 使用 `bypassPermissions`，TUI 新建进程时加 `--dangerously-skip-permissions` |
...
- 如果要让 TUI 以 `bypass` 启动，必须新建或 `replaceTui:true` 重建 pane；已经存在的 Claude Code TUI 不能事后改变启动权限参数。
